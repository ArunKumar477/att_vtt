<?php
	...
	if($con)
	{
		...
		if($exe)
		{
            ...
            $msg     .= '<table border="1" width="60%" style="border-collapse: collapse;margin-left:30px;">'; 
            $msg 	 .= '<tr style="background:skyblue;"><th> Fos Name </th><th> FTD Target </th><th> FTD Achieved </th><th> FTD Difference </th><th> FTD AchievedPerc </th><th> FTD CounterStock</th><th> MTD Target </th><th> MTD Achieved </th><th> MTD Difference </th><th> MTD AchievedPerc </th><th> MTD CounterStock</th></tr>';
			// $sql 	  = "select c.Fos_Name as FOS_Name,c.FTD_Target,c.FTD_Achieved,c.FTD_Difference,c.FTD_AchievedPerc,d.MTD_Target,d.MTD_Achieved,d.MTD_Difference,d.MTD_AchievedPerc from (select a.fos_name as FOS_Name,a.Target as FTD_Target,ifnull(b.achieved,0) as FTD_Achieved, ifnull((a.Target-b.achieved),0) as FTD_Difference,ifnull(round(((ifnull(b.achieved,0))/(a.Target)*100)),0) as FTD_AchievedPerc  from (SELECT app_users.fos_name,ifnull(round(count(shops.id)/3),0) as Target FROM app_users LEFT OUTER JOIN shops on app_users.id=shops.fos where app_users.un_fos<>1 and shops.Deleted='0'  group by app_users.user_name) a 
			// LEFT outer JOIN 
			// (select app_users.fos_name, ifnull(round(count(shop_id)),0) as Achieved from attendance left outer join app_users on attendance.fos=app_users.user_name where date(attendance_date)=(curdate() - INTERVAL 1 DAY) and app_users.un_fos<>1 group by fos) b
			// on a.fos_name=b.fos_name) c
			// LEFT OUTER JOIN
			// (select a.fos_name as FOS_Name,a.Target as MTD_Target,ifnull(b.achieved,0) as MTD_Achieved , ifnull((a.Target-b.achieved),0) as MTD_Difference,ifnull(round(((ifnull(b.achieved,0))/(a.Target)*100)),0) as MTD_AchievedPerc from (SELECT app_users.fos_name,ifnull(round((count(shops.id)/3)*(SELECT count(distinct date(attendance_date)) FROM `attendance` where (date(attendance_date) BETWEEN DATE_FORMAT(curdate() ,'%Y-%m-01') AND (curdate() - INTERVAL 1 DAY)) and (DAYOFWEEK(date(attendance_date))) <> 1)),0) as Target FROM app_users LEFT OUTER JOIN shops on app_users.id=shops.fos where app_users.un_fos<>1 and shops.deleted='0' group by app_users.user_name) a 
			// LEFT outer JOIN 
			// (select app_users.fos_name, ifnull(round(count(shop_id)),0) as Achieved from attendance left outer join app_users on attendance.fos=app_users.user_name where (date(attendance_date) between  DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) and app_users.un_fos<>1 group by fos) b
			// on a.fos_name=b.fos_name) d
			
			// on c.FOS_Name=d.FOS_Name";
			// $sql="select c.Fos_Name as FOS_Name,c.FTD_Target,c.FTD_Achieved,c.FTD_Difference,c.FTD_AchievedPerc,d.MTD_Target,d.MTD_Achieved,d.MTD_Difference,d.MTD_AchievedPerc from (select a.fos_name as FOS_Name,a.Target as FTD_Target,ifnull(b.achieved,0) as FTD_Achieved, ifnull((a.Target-b.achieved),0) as FTD_Difference,ifnull(round(((ifnull(b.achieved,0))/(a.Target)*100)),0) as FTD_AchievedPerc  from (SELECT app_users.fos_name,'20' as Target FROM app_users LEFT OUTER JOIN shops on app_users.id=shops.fos where app_users.active=1 and app_users.rights=0 and shops.Deleted='0' and app_users.id <> '15'  group by app_users.user_name) a 
			// LEFT outer JOIN 
			...
			// on a.fos_name=b.fos_name) c
			// LEFT OUTER JOIN
			// (select a.fos_name as FOS_Name,a.Target as MTD_Target,ifnull(b.achieved,0) as MTD_Achieved , ifnull((a.Target-b.achieved),0) as MTD_Difference,ifnull(round(((ifnull(b.achieved,0))/(a.Target)*100)),0) as MTD_AchievedPerc from (SELECT app_users.fos_name,ifnull((20*(SELECT count(distinct date(attendance_date)) FROM `attendance` where (date(attendance_date) BETWEEN DATE_FORMAT(curdate() ,'%Y-%m-01') AND curdate()) and (DAYOFWEEK(date(attendance_date))) <> 1)),0) as Target FROM app_users LEFT OUTER JOIN shops on app_users.id=shops.fos where app_users.active=1 and app_users.rights=0 and shops.deleted='0' and app_users.id <> '15' group by app_users.user_name) a 
			// LEFT outer JOIN 
            ...
            if($ex)
            {
                while($rs = mysqli_fetch_array($ex))
                {
					$msg     .= '<tr style="text-align:right;"><td style="padding:10px;text-align:left">'. $rs['FOS_Name'] .'</td><td style="padding:10px;">'. $rs['FTD_Target'] .'</td><td style="padding:10px;">'. $rs['FTD_Achieved'] .'</td><td style="padding:10px;">'. $rs['FTD_Difference'] .'</td><td style="padding:10px;">'. $rs['FTD_AchievedPerc'] .'</td>
					<td style="padding:10px;">'. $rs['FTD_CounterStock'] .'</td><td style="padding:10px;">'. $rs['MTD_Target'] .'</td><td style="padding:10px;">'. $rs['MTD_Achieved'] .'</td><td style="padding:10px;">'. $rs['MTD_Difference'] .'</td><td style="padding:10px;">'. $rs['MTD_AchievedPerc'] .'</td>
					<td style="padding:10px;">'. $rs['MTD_CounterStock'] .'</td></tr>';
                }
            }
			$msg	.= '</table>';
