<?php
	...
	if($con)
	{
		...
		/*$FOSRank_query = "SELECT FOS,FOSPercentage, @cnt:=@cnt+1 AS row_number from (select app_users.id as FOS,(fs.SFOSPerc+gb.BFOSperc+hc.FTDSFOSPerc+hc.MTDSFOSPerc+hc.FTDCFOSPerc+hc.MTDCFOSPerc) as FOSPercentage
			...
			$FOSRank_query = "select UserId,Name,SalesScore,CollectionScore,CounterStock,OverallScore, @cnt:=@cnt+1 AS row_number from (select app_users.id as UserId,app_users.fos_name as Name,round(fs.SFOSPerc+hc.FTDSFOSPerc+hc.MTDSFOSPerc) as SalesScore,round(hc.FTDCFOSPerc+hc.MTDCFOSPerc) as CollectionScore,ifnull(round((CASE WHEN (CS_MTD_AchievedPerc/100)*(select percentage from PP_Weightage where type='MTDB')> (select percentage from PP_Weightage where type='MTDB') THEN (select percentage from PP_Weightage where type='MTDB') ELSE ((CS_MTD_AchievedPerc/100)*(select percentage from PP_Weightage where type='MTDB')) END)),0) as CounterStock,(fs.SFOSPerc+ifnull(round((CASE WHEN ((CS_MTD_AchievedPerc/100)*(select percentage from PP_Weightage where type='MTDB'))> (select percentage from PP_Weightage where type='MTDB') THEN (select percentage from PP_Weightage where type='MTDB') ELSE ((CS_MTD_AchievedPerc/100)*(select percentage from PP_Weightage where type='MTDB')) END)),0)+hc.FTDSFOSPerc+hc.MTDSFOSPerc+hc.FTDCFOSPerc+hc.MTDCFOSPerc) as OverallScore
from (
	(SELECT FOS,ifnull((((SUM(TotalPerc)*100)*(SELECT percentage from PP_Weightage where type='SP'))/100),0) as SFOSPerc FROM (SELECT 
	a.fos as FOS,
	a.Model as Model,
	...
	(select app_users.fos_name, ifnull(round((count(shopid))),0) as Achieved,app_users.id as fos
	from stocks_c left outer join app_users 
	on stocks_c.userid=app_users.id 
	where date(datetime)=curdate()  
	and app_users.Active=1 
	...
	and shops.deleted='0'    group by app_users.fos_name) a 
	LEFT outer JOIN 
	(select app_users.fos_name, ifnull(round((count(shopid))),0) as Achieved,app_users.id 
	from stocks_h left outer join app_users on stocks_h.userid=app_users.id 
	where (date(datetime) between  DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) and app_users.Active=1 
	and app_users.rights=0  group by userid	) b
	on a.fos_name=b.fos_name)) total group by fos) gb on fs.fos=gb.fos
	...
	left OUTER join 
	(SELECT user_id as FOS,ifnull((sum(amount)),0) as Achieved FROM `invoice_payment` WHERE (ifnull(date(pymnt_date),date(created)) between  DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) group by user_id)e on d.fos=e.fos)FOSPerc left outer join app_users on FOSPerc.FOS=app_users.id where app_users.Active=1 and app_users.rights=0 GROUP by FOSPerc.FOS
	)hc on fs.fos=hc.fos left outer join app_users on fs.fos=app_users.id
left outer join 
(
SELECT c.fos_name as  fos_name, c.CS_FTD_Target, c.CS_FTD_Achieved, c.CS_FTD_AchievedPerc,
		d.CS_MTD_Target,
		...
		from
			...
			LEFT OUTER JOIN
					...
					ifnull(round(((ifnull(b.achieved,0))/(a.Target)*100)),0) as CS_MTD_AchievedPercent 
...
on q.fos_name=app_users.fos_name
) WHERE app_users.active=1 and app_users.rights=0 and fs.fos not in ('7','9')) d order by d.OverallScore desc";									
			...
			while($RankNumber=mysqli_fetch_array($FOSRankexe))
			{
				...
				$FOS=$RankNumber["UserId"];
				
				if($RankNumber["UserId"]==$id)
				{
					
					$rankFinal = $RankNumber["row_number"];
				}
			}
			$msg     .= '<p style="margin-left:30px;"><strong>Fos </strong> : '.$fosname.' ('.$fos.')     ';
				if($id!='7' && $id!='9' && $id!='15')
			$msg     .= '<strong>Rank </strong> : '.$rankFinal;
			$getWOD = mysqli_query($con,"select (SELECT count(DISTINCT Name) as total_shops FROM `shops` where fos=$id and Deleted=0 group by fos) as total_shops,(SELECT count(distinct s.particulars) as sales_shops FROM `sales` s,shops sh WHERE s.`particulars`=sh.Name and sh.fos=$id and sh.Deleted=0 and Month(sales_date) = Month(curdate()) and Year(sales_date)=Year(curdate()) group by sh.fos) as sales_shops from shops where fos=$id group by fos");
			if(mysqli_num_rows($getWOD)>0)
				$score  = mysqli_fetch_array($getWOD);
			$msg	 .= '<strong>&nbsp;&nbsp;&nbsp;WOD </strong> : '.$score["sales_shops"].' / '.$score["total_shops"];
			$msg     .= '</p>';
	...
	(select 'MTD',a.Target, ifnull(b.achieved1+bb.achieved2,0), ifnull((a.Target-(b.achieved1+bb.achieved2)),0),ifnull(round(((ifnull(b.achieved1+bb.achieved2,0))/(a.Target)*100)),0),ifnull(c.attendanceCount,0)  from (SELECT app_users.fos_name,ifnull((15*(SELECT count(distinct date(datetime)) FROM `stocks_c` where (date(datetime) BETWEEN DATE_FORMAT(curdate() ,'%Y-%m-01') AND curdate()) and (DAYOFWEEK(date(datetime))) <> 1)),0) as Target  FROM app_users LEFT OUTER JOIN shops on app_users.id=shops.fos where app_users.Active=1 and app_users.rights=0 and app_users.id=$id and shops.deleted='0' group by app_users.user_name) a 
	LEFT outer JOIN 
	(select app_users.fos_name, ifnull(round(count(shopid)),0) as Achieved1 from stocks_c left outer join app_users on stocks_c.userid=app_users.id where (date(datetime) between  DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) and app_users.id=$id and app_users.Active=1 and app_users.rights=0  group by userid) b
	on a.fos_name=b.fos_name
	LEFT OUTER JOIN
    (select app_users.fos_name, ifnull(round(count(shopid)),0) as Achieved2 from stocks_h left outer join app_users on stocks_h.userid=app_users.id where (date(datetime) between  DATE_FORMAT(NOW() ,'%Y-%m-01') AND NOW()) and app_users.id=$id and app_users.Active=1 and app_users.rights=0  group by userid) bb
	on b.fos_name=bb.fos_name
	LEFT OUTER JOIN
	(SELECT app_users.fos_name,count(attendance.shop_id) as attendanceCount
	from app_users RIGHT outer join attendance on app_users.user_name=attendance.fos
