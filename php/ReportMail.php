<?php
	...
	if($con)
	{
		...
		if($exe)
		{
			...
			UNION
			SELECT 'FTD' as Name,
			ifnull(round(sum(a.AmtaboveCPoutb)/25),0) as Target,
			ifnull(round(b.AchievedOverdue),0) as Achieved ,
			ifnull(round(sum(a.AmtaboveCPoutb)/25 - b.AchievedOverdue),0) as Difference,
			ifnull(round(b.AchievedOverdue/sum(a.AmtaboveCPoutb/25)*100),0) as AchievedPerc 
